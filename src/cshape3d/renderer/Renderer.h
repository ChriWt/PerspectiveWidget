...
private:
  ...
  std::vector<std::tuple<int, int>> startRendering() {
    ...
    std::vector<std::tuple<int, int>> connectinVertices =
        shape.get()->getEdges();
  ...
  Vector3 projectVertex(Vector3 vertex) {
    ...
    Vector4 vertex4d =
        Vector4(centeredVetex.x, centeredVetex.y, centeredVetex.z, 1);
    ...
    return Vector3(projectedVertex.x * scale + offsetX,
                   projectedVertex.y * scale + offsetY, projectedVertex.z);
  }
  ...
  Matrix4x4 getRotationMatrixX() {
    if (!rotationMatrixXInitialized) {
      ...
      this->rotationMatrixX = {{{cos(radian), 0, sin(radian), 0},
                                {0, 1, 0, 0},
  ...
  Matrix4x4 getRotationMatrixZ() {
    if (!rotationMatrixZInitialized) {
      ...
      this->rotationMatrixZ = {{{cos(radian), -sin(radian), 0, 0},
  ...
  Vector3 normalizeProjection(Vector4 vertex) {
    if (vertex.w != 0.0f) {
      ...
      std::cerr << "Warning: Vertex has a w-component of 0. Returning "
                   "unmodified vertex."
                << std::endl;
  ...
  void buildProjectionMatrix() {
    float scaleX =
        (1.0f / tan(radians(this->renderingProperties.getFov() / 2.0f))) *
        this->renderingProperties.getAspectRatio();
    float scaleY =
        1.0f / tan(radians(this->renderingProperties.getFov() / 2.0f));
    float scaleZ = ((this->renderingProperties.getFar() +
                     this->renderingProperties.getNear()) *
                    -1.0f) /
                   (this->renderingProperties.getFar() -
                    this->renderingProperties.getNear());
    float translate_z = (-2 * this->renderingProperties.getFar() *
                         this->renderingProperties.getNear()) /
                        (this->renderingProperties.getFar() -
                         this->renderingProperties.getNear());

    this->projectionMatrix = {{{scaleX, 0, 0, 0},
                               {0, scaleY, 0, 0},
                               {0, 0, scaleZ, translate_z},
                               {0, 0, -1, 0}}};
  }

  float radians(int degree) { return degree * (M_PI / 180); }
...
public:
  ...
  setRenderingPropertiesAndRender(RenderingProperties renderingProperties) {
